...
| `music_live_cross.py` | Crosses a `nowplaying_wide_probe` capture against a *concurrent* deep-analysis log and counts song-minutes where the in-band segment says **music is airing** (`song_spot=M`, title or not) while the committed display read LIVE - separating "no channel named a song" from "no song was playing" (every title-based measure collapses those). Broken out by whether any channel had a title (nameable) vs none did (music playing, zero titles anywhere, screen LIVE - the pure form), with the raw segment tag at each instant and episodes >=30s. |
| `segment_anomaly.py` | Infers **song-slopped-into-an-ad-tag** from SHAPE, not the label, over deep logs: computes data-derived baselines (song span, real ad-pod duration, spot-id churn) then flags long non-music stretches that are churn-free with `mfSeq` flowing (song-shaped, no ad-pod structure). Corroborates every flag against the runtime log's segment-level `cls=` (RE-MOCT's own `song_spot` read): `cls=music` under a non-music tag = the bug; `other` = genuine talk (benign). Auto-locates `remoct-<date>.log` beside the deep log. |
| `anomaly_state_cross.py` | Joins `segment_anomaly`'s long confirmed-talk stretches against the runtime log's `LIVE-floor stall ... re-pin` fire lines + deep-log `connectSeq` increments, to answer whether the re-pin/floor logic **thrashes during talk** (fires repeatedly trying to escape a "pod" that is actually the show) or rides it out. Per-stretch QUIET/THRASHING verdict, fire count, cadence, and total spurious repins. This is the instrument that located the F6 talk-thrash bug fixed in 1.3.1. |
